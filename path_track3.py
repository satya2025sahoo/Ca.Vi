...
def load_images(folder_path, max_frames):
    """Load images with validation, limited to max_frames"""
    images = []
    if not os.path.exists(folder_path):
        print(f"Error: Folder {folder_path} does not exist")
        return images, []
    
    filenames = sorted([f for f in os.listdir(folder_path) if f.lower().endswith(('.jpg', '.png'))])[:max_frames]
...
def estimate_trajectory(images, frame_files, K, collision_log_path, output_dir="path_track"):
    ...
    # Load collision log and extract status column
    collision_df = pd.read_csv(collision_log_path)
    status_list = collision_df['status'].tolist()  # Extract status column as list
    ...
    for i in tqdm(range(min(len(images)-1, max_frames-1)), desc="Processing frames"):
        frame_idx = int(frame_files[i].split('_')[1].split('.')[0])
        output_path = os.path.join(output_dir, f"path_{frame_idx:05d}.jpg")
        
        # Check collision status from status_list
        if frame_idx < len(status_list) and status_list[frame_idx] == "STOP":
        ...
        # Analyze motion type
        motion_type, angle, is_obstacle = analyze_motion(R, t, motion_history)
        
        # Apply motion constraints with stabilization after resume
        if motion_type != "straight" and angle > 45 and i > 0:  # Stabilize after resume
    ...
    if len(trajectory) > SMOOTHING_WINDOW:
        ...
        for i in range(3):
            window = 5 if len(turn_info) > 0 or len(obstacle_info) > 0 else SMOOTHING_WINDOW
            trajectory[:,i] = savgol_filter(trajectory[:,i], window, 2)
...
def main(frames_folder="frames", collision_log_path="output/frame_status.csv"):
    ...
    trajectory, turn_info, obstacle_info = estimate_trajectory(images, frame_files, K, collision_log_path)
    end_time = time.time()
